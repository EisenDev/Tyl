<template>
  ...
  <Transition name="main-bloom">
    <div v-if="unlocked" class="site-root">
      <!-- Scroll Progress -->
      <div v-if="!isProposalRoute" class="scroll-progress" :style="{ width: scrollProgress + '%' }" />

      <!-- Floating Tulip (parallax) -->
      <FloatingTulip v-if="!isProposalRoute" />

      <!-- Navigation -->
      <SiteNav v-if="!isProposalRoute" />
...
const isProposalRoute = computed(() => route.path === '/my-proposal')
