<template>
  <!-- Hidden YouTube player containers (1px off-screen) -->
  <div
...
/* ── YouTube IFrame API ────────────────────────────────────────── */
function loadYTApi() {
  if ((window as any).YT?.Player) { initPlayers(); return }
  ...
  tracks.forEach((track) => {
    players[track.id] = new YT.Player(`yt-${track.id}`, {
      ...
      playerVars: {
        ...
        loop: 1, playlist: track.videoId,
        rel: 0, fs: 0, modestbranding: 1, enablejsapi: 1,
...
/* ── Exposed: App.vue calls this after unlock ─────────────────── */
function startFade() {
...
function onOutsideClick(e: MouseEvent) {
  if (showPicker.value && !(e.target as HTMLElement).closest('#audio-track-btn, .track-picker')) {
    showPicker.value = false
  }
}
...
.audio-label {
  ...
  color: var(--ink-muted); opacity: 0; transform: translateX(6px);
...
/* Visualizer */
.audio-visualizer { display: flex; align-items: flex-end; gap: 2px; height: 18px; }
.viz-bar { display: block; width: 3px; border-radius: 2px; background: var(--sage-light); height: 4px; transform-origin: bottom; }
.audio-visualizer:not(.playing) .viz-bar { height: calc(3px + var(--i) * 1px); opacity: 0.35; }
...
.audio-toggle {
  ...
  box-shadow: 0 4px 16px rgba(0,0,0,0.07); transition: border-color 0.3s, box-shadow 0.3s;
...
.track-btn {
  ...
  background: rgba(250, 249, 246, 0.88); border: 1px solid var(--parchment);
  display: flex; align-items: center; justify-content: center; cursor: pointer;
  box-shadow: 0 2px 8px rgba(0,0,0,0.06); transition: border-color 0.25s, box-shadow 0.25s, background 0.25s;
  backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); flex-shrink: 0;
}
.track-btn:hover, .track-btn.active {
  border-color: var(--sage); box-shadow: 0 3px 14px rgba(134,167,137,0.25);
...
.track-picker {
  ...
  border-radius: 14px; padding: 1rem 0.8rem 0.8rem; min-width: 230px;
...
.track-option {
  display: flex; align-items: center; gap: 0.7rem; width: 100%; padding: 0.65rem 0.7rem;
  border-radius: 10px; border: none; background: transparent; cursor: pointer;
  transition: background 0.2s; text-align: left; color: var(--ink);
}
